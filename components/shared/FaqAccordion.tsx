      ...
      {items.map((item, i) => (
        ...
        >
          <button
            ...
            className={`flex w-full items-center justify-between px-6 py-5 text-left text-sm font-medium transition-colors ${hoverBg} ${useActiveColor && openIndex === i ? 'text-corematrix-green400' : 'text-corematrix-textPrimary'
              }`}
          >
            <span>{item.q}</span>
            <svg
              className={`ml-4 h-5 w-5 shrink-0 ${iconColor} transition-transform duration-300 ${openIndex === i ? 'rotate-180' : ''
                }`}
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
          ...
          <div
            className={`overflow-hidden transition-all duration-300 ${openIndex === i ? maxH : 'max-h-0'
              }`}
          >

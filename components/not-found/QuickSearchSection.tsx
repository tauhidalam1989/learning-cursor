...
export function QuickSearchSection() {
  ...
  const [filteredLinks, setFilteredLinks] = useState(QUICK_LINKS);

  useEffect(() => {
    if (!query.trim()) {
      setFilteredLinks(QUICK_LINKS);
      return;
    }
    setFilteredLinks(
    ...
    >
      <Container>
        ...
        <p className="mt-2 text-sm text-corematrix-textMuted">
          Search or browse common pages to find what you need.
        </p>
        ...
        <div className="mt-4 flex flex-wrap justify-center gap-2">
          {filteredLinks.length > 0 ? (
            ...
            <p className="text-sm text-corematrix-textDim">No matches found</p>
          )}
        </div>
      </Container>
    </section>
